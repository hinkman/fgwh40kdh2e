...
function onInit()
	...
	DB.addHandler(sNode .. ".ammocount", "onUpdate", onAmmoChanged);
	onAmmoChanged();
	
	maxammonode = window.getDatabaseNode().createChild("clipsize", "number");
...
function onClose()
	local sNode = window.getDatabaseNode().getNodeName();
	DB.removeHandler(sNode .. ".ammocount", "onUpdate", onAmmoChanged);
...
function getMaxValue()
	return DB.getValue(window.getDatabaseNode(), "clipsize", 0);
end

function getCurrentValue()
	return DB.getValue(window.getDatabaseNode(), "ammocount", 0);
end
...
function setCurrentValue(nCount)
	DB.setValue(window.getDatabaseNode(), "ammocount", "number", nCount);
end
