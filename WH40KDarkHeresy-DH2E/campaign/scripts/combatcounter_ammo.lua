...
slots = {};
local nMaxSlotRow = 60;
local nDefaultSpacing = 9;
local nSpacing = nDefaultSpacing;

function onInit()
	local sNode = window.getDatabaseNode().getNodeName();
	DB.addHandler(sNode .. ".combatammocount", "onUpdate", onAmmoChanged);
	onAmmoChanged();
	
	maxammonode = window.getDatabaseNode().createChild("combatclipsize", "number");
...
function onClose()
	local sNode = window.getDatabaseNode().getNodeName();
	DB.removeHandler(sNode .. ".combatammocount", "onUpdate", onAmmoChanged);
...
function getMaxValue()
	return DB.getValue(window.getDatabaseNode(), "combatclipsize", 0);
end

function getCurrentValue()
	return DB.getValue(window.getDatabaseNode(), "combatammocount", 0);
end
...
function setCurrentValue(nCount)
	DB.setValue(window.getDatabaseNode(), "combatammocount", "number", nCount);
end
